...
public partial class Step22ViewModel : StepViewModelBase
{
    ...
    private async Task BasicDemo()
    {
        ...
        async (id, ct) =>
        {
            ...
            Log($"   ? Item {id,2} processed ({delay}ms) " +
                $"[Thread: {Environment.CurrentManagedThreadId}]");
        });

        sw.Stop();
        Log($"\n   ?? Total time: {sw.ElapsedMilliseconds}ms");
        Log("   ?? Only 3 items ran at a time. Much faster than sequential!\n");
        IsRunning = false;
    }
    ...
    private async Task CompareDemo()
    {
        ...
        Log($"   ?? Sequential time: {seqTime}ms\n");

        // --- Parallel ---
        sw.Restart();
        Log("   ? Parallel.ForEachAsync (max 3 concurrent):");
        ...
        Log($"   ?? Parallel time: {parTime}ms");
        Log($"\n   ?? Parallel was ~{seqTime / Math.Max(parTime, 1)}x faster!");
        Log($"   ?? {seqTime}ms ? {parTime}ms\n");
        IsRunning = false;
    }
    ...
    private async Task WithCancellation(CancellationToken token)
    {
        ...
        {
            Log("\n   ?? Cancelled! Remaining items were not processed.\n");
        }
        finally
        {
            IsRunning = false;
